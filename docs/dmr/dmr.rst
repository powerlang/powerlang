...
The SExpressions are AOT nativized, so that all
needed code can be executed and, when required, a JIT compiler can be loaded to that
image so that new code can be nativized on-the-fly.

Memory and garbage collector are written in the same language than the rest of the
